...
public abstract class GraphSolver {
	...
	public static boolean[] sequential_addition_dist(Graph G, int k, boolean notify) {
		...
		double[] dist = Graph_Util.dijkstra_default(sel, G);
		...
		if (notify) {
			...
			try {
				...
				for (int i = 1; i < k; i++) {
					dist = Graph_Util.dijkstra_two_source_update(G.get_nodes().get(prev_sel_1),
	...
	public static boolean[] k_seq_add_dist(boolean[] sel, Graph G, int k) {
		...
		while (i < k) {
			dist = Graph_Util.dijkstra_two_source_update(G.get_nodes().get(prev_sel_1), G.get_nodes().get(prev_sel_2),
					dist, G);
	...
	public static boolean[] sequential_addition_gap(Graph G, int k, boolean notify) {
		...
		double[] dist = Graph_Util.dijkstra_gap_eval(sel, G);
		...
		if (notify) {
			try {
				...
				for (int i = 1; i < k; i++) {
					dist = Graph_Util.dijkstra_two_source_update(G.get_nodes().get(prev_sel_1),
							G.get_nodes().get(prev_sel_2), dist, G);
					Edge max_len = Graph_Util.evaluate_selection(sel, G, dist).first;
					prev_sel_1 = max_len.first;
					prev_sel_2 = max_len.second;
					sel[max_len.index] = true;
					pb.step();
	...
	public static boolean[] sequential_addition_gap_p(Graph G, int k, boolean notify) {
		...
		while (i < k) {
			...
			Thread t = new Thread(() -> {
				Graph_Util.dijkstra_single_source_update(G.get_nodes().get(index), dist_1, G);
			});
			t.start();
			dist_2 = Graph_Util.dijkstra_single_source_update(G.get_nodes().get(prev_sel_2), dist_2, G);
			try {
				t.join();
	...
	public static boolean[] one_opt(Graph G, boolean[] sel) {
		...
		double curr_max_gap_length = Graph_Util.evaluate_selection(sel, G).second;
		while (has_changed) {
			has_changed = false;
			double[] dist = Graph_Util.dijkstra_gap_eval(sel, G);
			for (int i = 0; i < sel.length; i++) {
				if (sel[i]) {
					...
					dist = Graph_Util.dijkstra_two_source_removal_upd(sel, G, dist, e, curr_max_gap_length);
					Tupel<Edge, Double> t_0 = Graph_Util.evaluate_selection(sel, G, dist);
					int j = t_0.first.index;
					if (j != i) {
						sel[j] = true;
						dist = Graph_Util.dijkstra_two_source_update(G.get_nodes().get(G.get_edges().get(j).first),
								G.get_nodes().get(G.get_edges().get(j).second), dist, G);
						double new_max_gap_length = Graph_Util.evaluate_selection(sel, G, dist).second;
		...
		while (has_changed) {
			...
			for (int i = 0; i < sel.length; i++) {
				if (sel[i]) {
					sel[i] = false;
					Edge e = G.get_edges().get(i);
					dist = Graph_Util.dijkstra_gap_eval(sel, G);
					Tupel<Edge, Double> t_0 = Graph_Util.evaluate_selection(sel, G, dist);
					int j = t_0.first.index;
					if (j != i) {
						sel[j] = true;
	...
	public static boolean[] k_seq_add_gap(boolean[] sel, Graph G, int k) {
		...
		Edge start_edge = Graph_Util.evaluate_selection(sel, G, dist).first;
		sel[start_edge.index] = true;
		int prev_sel_1 = start_edge.first;
		int prev_sel_2 = start_edge.second;
